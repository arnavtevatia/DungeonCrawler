...
public interface SeekState {
	...
	boolean isLookingForExit();   // Whether the player is looking for the exit
	boolean isLookingForTreasure(); // Whether the player is looking for treasure

	private int heuristic(long currentLocation, long ringLocation) {
		int currentRow = (int) (currentLocation >> 32);
		int currentCol = (int) currentLocation;
		int ringRow = (int) (ringLocation >> 32);
		int ringCol = (int) ringLocation;
		return Math.abs(currentRow - ringRow) + Math.abs(currentCol - ringCol);
	}
