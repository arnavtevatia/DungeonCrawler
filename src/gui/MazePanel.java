...
public class MazePanel extends JPanel {
    ...
    public void paintComponent(Graphics page) {
        ...
        for (int row = 0; row < sewers.rowCount(); row++) {
            for (int col = 0; col < sewers.columnCount(); col++) {
                ...
                // Highlight tile if visited and part of the path
                if (visited[row][col]) {
                    page.setColor(new Color(255, 255, 0, 128)); // Yellow with transparency
    ...
    public void highlightAndMovePlayer(List<Tile> path) {
        for (Tile t : path) {
            int row = t.row();
            int col = t.column();
            ...
            try {
                Thread.sleep(200); // Pause for visualization (adjust as needed)
